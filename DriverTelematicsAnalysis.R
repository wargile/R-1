...
# https://www.kaggle.com/c/axa-driver-telematics-analysis/forums/t/11283/beat-the-benchmark-with-journey-distance-in-r
# https://github.com/saihttam/kaggle-axa
# http://www.kaggle.com/c/axa-driver-telematics-analysis/forums/t/12299/any-suggestions-on-doing-local-evaluations
# https://github.com/mariopasquato/kaggledriversvisual/blob/master/visua.R
# Dynamic Time Warping (DTW) tip:
...
# Draw repeated trips:
# http://www.kaggle.com/blobs/download/forum-message-attachment-files/1914/draw_repeated_trip.py
# https://www.kaggle.com/c/axa-driver-telematics-analysis/forums/t/12849/github-repos-now-live

# From Admin: "The metric is AUC, so you can submit either continuous probabilities or binary predictions.
# You will happen to do a lot better if you choose the former."

# From https://www.kaggle.com/c/axa-driver-telematics-analysis/forums/t/11299/score-0-66-with-logistic-regression?page=4:
# "You'll need 3+ (x,y) points and their associated velocity vector"

# # See also: convolve, fft, filter, time series logic, arima, etc.

# In pseudocode, this looks as follows.
# Create a reference set of trips: 1000 trips from 1000 different drivers
# For each driver (you want to test): take the 200 trips of current driver (or subsample thereof) as positive training examples (label=1), the reference trips (same for all drivers) as negative ones (label=0)
# I include k random outliers, i.e. k trip from k other drivers and include them with label=1 in the set of the current driver. So my training set has 200+k+1000 samples.
# I fit my model (e.g. Logistic Regression) to the data, the labels are 200+k ones and 1000 zeros
# The model returns the probabilities of the trips belonging to class 1, they are stored
# The indices of the artificial outliers are stored in a vector
...
dataFolder <- "C:/coding/Kaggle/DriverTelematicsAnalysis/data/drivers/drivers/1004/"
codeFolder <- "C:/coding/Kaggle/DriverTelematicsAnalysis/code/"
submissionsFolder <- "C:/coding/Kaggle/DriverTelematicsAnalysis/submissions/"

# TODO:
# - Build feature set (random forest gives best result): Distance, trip length, speed, velocity, acceleration, etc.
#   Extract quantiles from the total data set of the various measurement types?
# - Add drivers from other folders to test against
...
GetRandomDrivers <- function() {
  ...
  for (counter in file.from:file.to) {
    ...
    summary(train)
    sapply(train, class)
    plot(train, type="o", col="blue", cex.lab=.7, cex.axis=.7, asp=1)
